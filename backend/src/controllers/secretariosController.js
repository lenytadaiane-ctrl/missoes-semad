...
async function list(req, res, next) {
  try {
    ...
    const items = await prisma.secretarioMissoes.findMany({ where, include: { pessoa: true, setor: true }, orderBy: { pessoa: { nome: 'asc' } } });
    res.json(items);
  } catch (err) { next(err); }
}
...
async function create(req, res, next) {
  try {
    ...
    const item = await prisma.secretarioMissoes.create({
...
async function update(req, res, next) {
  try {
    ...
    const { setorId, dataInicio, ...rest } = req.body;
    const existing = await prisma.secretarioMissoes.findUnique({ where: { id } });
    if (!existing) return res.status(404).json({ error: 'Secretário não encontrado' });
    const pessoaData = buildPessoa(rest);
    await prisma.pessoa.update({ where: { id: existing.pessoaId }, data: pessoaData });
    const item = await prisma.secretarioMissoes.update({
      where: { id },
...
async function remove(req, res, next) {
  try {
    await prisma.secretarioMissoes.delete({ where: { id: parseInt(req.params.id) } });
    res.status(204).send();
  } catch (err) { next(err); }
}
