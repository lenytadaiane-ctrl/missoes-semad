...
export default function FormSecretario() {
  const { id } = useParams();
  const isEdit = !!id;
  ...
  useEffect(() => {
    if (record) reset({ ...record.pessoa, setorId: record.setorId, dataInicio: record.dataInicio?.slice(0, 10) || '', dataNascimento: record.pessoa?.dataNascimento?.slice(0, 10) || '', congregacaoOrigemId: record.pessoa?.congregacaoOrigemId || '' });
  }, [record, reset]);

  async function onSubmit(data) {
    try {
      if (isEdit) { await api.put(`/secretarios-missoes/${id}`, data); toast.success('Atualizado'); }
      else { await api.post('/secretarios-missoes', data); toast.success('Cadastrado'); }
  ...
  return (
    <div className="max-w-2xl space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">{isEdit ? 'Editar Secretário' : 'Novo Secretário'}</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
        ...
        <div className="flex gap-3">
          ...
          <Button type="button" variant="secondary" onClick={() => navigate('/secretarios')}>Cancelar</Button>
        </div>
      </form>
    </div>
  );
