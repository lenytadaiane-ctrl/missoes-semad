...
export default function FormAgente() {
  ...
  async function onSubmit(data) {
    try {
      if (isEdit) { await api.put(`/agentes-missoes/${id}`, data); toast.success('Atualizado'); }
      else { await api.post('/agentes-missoes', data); toast.success('Cadastrado'); }
      navigate('/agentes');
  ...
  return (
    <div className="max-w-2xl space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">{isEdit ? 'Editar Agente' : 'Novo Agente'}</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
        ...
        <div className="flex gap-3">
          ...
          <Button type="button" variant="secondary" onClick={() => navigate('/agentes')}>Cancelar</Button>
        </div>
      </form>
    </div>
  );
