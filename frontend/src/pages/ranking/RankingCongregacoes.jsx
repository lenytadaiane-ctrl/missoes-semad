...
function Delta({ delta }) {
  ...
  if (delta > 0) return (
    <span className="inline-flex items-center justify-center gap-0.5 text-emerald-600 text-xs font-bold">
  ...
  return (
    <span className="inline-flex items-center justify-center gap-0.5 text-rose-500 text-xs font-bold">
...
export default function RankingCongregacoes() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['ranking-congregacoes', ano, mes, setorId, tipo],
    queryFn: () => api.get('/ranking/congregacoes', { params: { ano, mes: mes || undefined, setorId: setorId || undefined } }).then(r => r.data),
  });

  const filtrado = tipo ? (data?.data || []).filter(r => r.tipo === tipo) : (data?.data || []);

  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Ranking por Congregação</h1>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <table className="min-w-full divide-y divide-gray-100 text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Congregação</th>
          ...
          <tbody className="divide-y divide-gray-50">
            {isLoading ? (
              ...
              <tr key={r.id} className={`hover:bg-gray-50 transition-colors ${i < 3 && r.total > 0 ? 'font-medium' : ''}`}>
                <td className="px-4 py-3">
                  <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold ${medalha(i + 1)}`}>
                    {i + 1}
                  </span>
                </td>
                <td className="px-4 py-3 text-gray-800">{r.nome}</td>
                <td className="px-4 py-3 text-gray-500 text-xs">{r.setor?.nome || '—'}</td>
                ...
                <td className="px-4 py-3 text-right font-semibold text-gray-800">{fmt(r.total)}</td>
        ...
        <div className="px-4 py-2 text-xs text-gray-400 border-t">
          {filtrado.length} congregação(ões) com arrecadação em {labelAtual}
        </div>
      </div>
    </div>
  );
